...
import (
	...
	"github.com/marcostota/chatservice/internal/domain/entity"
	"github.com/marcostota/chatservice/internal/gateway"
...
func NewChatCompletionUseCase(chatGateway gateway.ChatGateway, openAIClient *openai.Client,
	stream chan ChatCompletionOutputDTO) *ChatCompletionUseCase {
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	useMessage, err := entity.NewMessage("user", input.UserMessage, chat.Config.Model)
	if err != nil {
		return nil, errors.New("error creating new message " + err.Error())
	}
	err = chat.AddMessage(useMessage)
	if err != nil {
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	intialMessage, err := entity.NewMessage("system", input.Config.InitialSystemMessage, model)
	if err != nil {
		return nil, errors.New("error creating initial message: " + err.Error())
	}
	chat, err := entity.NewChat(input.UserID, intialMessage, chatConfig)
	if err != nil {
		return nil, errors.New("error creating new chat: " + err.Error())
	}
	return chat, nil
